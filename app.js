...
function openHistory(record){const events=[...(record.historialCambios||[])].sort((a,b)=>String(b.fecha).localeCompare(String(a.fecha)));$('#historyTitle').textContent=`Historial · Económico ${record.economico}`;$('#historyList').innerHTML=events.length?events.map(item=>`<article class="history-item" tabindex="0"><div><h3>${esc(item.titulo)}</h3><p>${esc(item.detalle||'')}</p><time>${new Intl.DateTimeFormat('es-MX',{dateStyle:'medium',timeStyle:'short'}).format(new Date(item.fecha))}</time></div></article>`).join(''):'<div class="history-empty">Aún no hay cambios registrados para este económico.<br />Los próximos movimientos quedarán guardados aquí.</div>';$('#historyDialog').showModal()}
