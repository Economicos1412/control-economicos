...
function openHistory(record){const events=[...(record.historialCambios||[])].sort((a,b)=>String(b.fecha).localeCompare(String(a.fecha))),labels={economico:'Número económico',marca:'Unidad / marca',modelo:'Modelo',serie:'Número de serie',categoria:'Categoría',folio:'Folio',monto:'Monto',area:'Área',fecha:'Fecha de prueba',estado:'Estado',observaciones:'Observaciones'},detail=item=>{if(!item.detalle?.startsWith('Campos modificados:'))return item.detalle||'';const fields=item.detalle.replace('Campos modificados:','').replace('.','').split(',').map(field=>field.trim());return fields.map(field=>`${labels[field]||field}: ${historyValue(field,record[field])}`).join(' · ')};$('#historyTitle').textContent=`Historial · Económico ${record.economico}`;$('#historyList').innerHTML=events.length?events.map(item=>`<article class="history-item" tabindex="0"><div><h3>${esc(item.titulo)}</h3><p>${esc(detail(item))}</p><time>${new Intl.DateTimeFormat('es-MX',{dateStyle:'medium',timeStyle:'short'}).format(new Date(item.fecha))}</time></div></article>`).join(''):'<div class="history-empty">Aún no hay cambios registrados para este económico.<br />Los próximos movimientos quedarán guardados aquí.</div>';$('#historyDialog').showModal()}
